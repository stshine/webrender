...
use api::{DeviceIntPoint, DeviceIntRect, DeviceUintSize, FontRenderMode, LayoutToDeviceScale};
use api::{DevicePoint, DeviceRect, DeviceSize};
use api::{ImageFormat, TextureTarget};
use api::{Shape, ShapeKind};
use debug_colors;
use device::{Device, Texture, TextureFilter, VAO};
use euclid::{Point2D, Rotation2D, Size2D, Transform3D, TypedVector2D, Vector2D};
use internal_types::RenderTargetInfo;
use lyon_path::PathEvent;
use pathfinder_partitioner::mesh::Mesh;
use pathfinder_partitioner::{FillRule, partitioner::Partitioner};
...
pub struct PathsCacheKey(pub u64);
...
pub struct VectorRasterizer {
    ...
    next_paths_cache_key: PathsCacheKey,
}
...
pub fn build_mesh(shapes: Vec<Shape>, scale: LayoutToDeviceScale) -> Vec<Mesh> {
    shapes.for_each(|shape| {
        let mut partitioner = Partitioner::new();
        match shape.kind {
            ShapeKind::Circle(c, r) => {
                let center = scale.transform_point(c);
                let radii = scale.0 * r;
                let iter = build_circle_paths(center, r).iter();
                iter.for_each(|event| partitioner.builder_mut.path_event(event));

                partitioner.partition(FillRule::Winding);
                partitioner.builder_mut().build_and_reset();

                partitioner.mesh_mut().push_stencil_segments(iter().cloned());
                partitioner.mesh_mut().push_stencil_normals(iter().cloned());
            }
            _ => {}
        }
        partitioner.into_mesh()
    })
}

fn build_circle_paths(center: DevicePoint, radius: f32) -> Vec<PathEvent> {
    let center = center.to_untyped();
    let radius = Vector2D::new(radius, radius);
    0..3.map(|i| PathEvent::Arc(
        center,
        radius,
        f32::consts::PI / 2,
        f32::consts::PI * i / 2,
